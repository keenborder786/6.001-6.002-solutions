...
class PlaintextMessage(Message):
    ...
    def get_encryption_dict(self):
        ...
        '''
        encryption_dic=self.encryption_dict
        return encryption_dic.copy()
    ...
    def change_shift(self, shift):
        ...
        Returns: nothing
        '''
        assert 0<=shift<=26, "Please enter a valid shift value"
        text=PlaintextMessage.get_message_text(self)
...
class CiphertextMessage(Message):
    ...
    def decrypt_message(self):
        ...
        for shift in range(26):
            valid_word_list=[]
            decrypted_message_text=""
            dic_words=CiphertextMessage.build_shift_dict(self,26-shift)
            for char in text:
                if char in alpha_lower:
                    index=dic_words[char]
                    old_word=alpha_lower[index]
                    decrypted_message_text+=old_word
                elif char in alpha_higher:
                   index_2=dic_words[char]
                   old_word_2=alpha_higher[index_2]
                   decrypted_message_text+=old_word_2
                else:
                    decrypted_message_text+=char
            for word in decrypted_message_text.split():
                if is_word(self.valid_words, word)==True:
                   valid_word_list.append(word)
                   i=len(valid_word_list)
            if i > imax:
                imax=i
                valid_decryption=decrypted_message_text 
        return valid_decryption
s=Message("I go to school and then sleep there.!")
a=s.apply_shift(3)
print(a)
b=CiphertextMessage(a)
