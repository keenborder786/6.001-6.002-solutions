...
class SimpleBacteria(object):
    ...
    def is_killed(self):
        ...
        """
        if self.death_prob>=random.uniform(0,1.):
            return True
        else:
            return False
    ...
    def reproduce(self, pop_density):
        ...
        """
        prob_birth=self.birth_prob*(1-pop_density)
        if prob_birth>=random.uniform(0,1):
...
class Patient(object):
    ...
    def update(self):
        ...
        """
        off_spring=[]
        ###Looping over to check if bacteria survives
        for bact in self.bacteria:
                if bact.is_killed()==True:
                   self.bacteria.remove(bact)
        current_bacteria_population=len(self.bacteria)
        pop_density=current_bacteria_population/self.max_pop
        for bact in self.bacteria:
                if bact not in off_spring and current_bacteria_population<=self.max_pop:
                    try:
                        new_bacteria=bact.reproduce(pop_density)
                        self.bacteria.append(new_bacteria)
                        off_spring.append(new_bacteria)
                        current_bacteria_population=len(self.bacteria)
...
def calc_pop_avg(populations, n):
    ...
    for trial in populations:
        trial_value_n.append(trial[n])
    return sum(trial_value_n)/len(trial_value_n)
...
def simulation_without_antibiotic(num_bacteria,
    ...
    Returns:
        populations (list of lists or 2D array): populations[i][j] is the
            number of bacteria in trial i at time step j
    """
    bacteria_count_ntimestep=[]
    populations=[]
    for trial in range(num_trials):
        bacteria_count_ntimestep=[]
        bacteria=[SimpleBacteria(birth_prob,death_prob) for n in range(num_bacteria)]
        patient=Patient(bacteria,max_pop)
        num_bacteria_2=num_bacteria
        for n in range(300):
            if n==0:
                bacteria_count_ntimestep.append(num_bacteria_2)
            else:
                num_bacteria_2=patient.update()
                bacteria_count_ntimestep.append(num_bacteria_2)
        populations.append(bacteria_count_ntimestep)
    avg_pop=[calc_pop_avg(populations,n) for n in range(300)]
    make_one_curve_plot(np.array(range(n+1)),np.array(avg_pop),"Timestep","Average Population","Without Antibotic")
    return populations
...
def calc_pop_std(populations, t):
    ...
    """
    summation=0
    mean_tstep=calc_pop_avg(populations, t)
    points_taken=[]
    for trial in populations:
        points_taken.append(trial[t])
    print(points_taken)
    for i in points_taken:
        summation=summation+(i-mean_tstep)**2
    std=(summation*(1/len(points_taken)))**(1/2)
    return std
...
def calc_95_ci(populations, t):
    ...
    points_taken=[]
    mean_tstep=calc_pop_avg(populations, t)
    std_tstep=calc_pop_std(populations, t)
    for trial in populations:
        points_taken.append(trial[t]) 
    sem=std_tstep/((len(points_taken))**(1/2))
    width=1.96*sem
    result=(mean_tstep,width)
    return result
#calc_avg, calc_ci_95 = calc_95_ci(population, 299)
#print(calc_ci_95)
##########################
# PROBLEM 4
##########################
...
class ResistantBacteria(SimpleBacteria):
    ...
    def is_killed(self):
        ...
        if self.resistant==True:
            ...
            if ((self.death_prob)/4)>=random.uniform(0,1):
                return True
            else:
                return False
    ...
    def reproduce(self, pop_density):
        ...
        if birth_prob>=random.uniform(0,1):
            if self.resistant==True:
                return ResistantBacteria(self.birth_prob,self.death_prob,True,self.mut_prob)
            elif self.resistant==False:
                if self.mut_prob * (1-pop_density)>=random.uniform(0,1):
                    return ResistantBacteria(self.birth_prob,self.death_prob,True,self.mut_prob)
                else:
...
class TreatedPatient(Patient):
    ...
    def update(self):
        ...
        if self.on_antibiotic==True:
            for bact in self.bacteria:
                if bact.resistant==False:
                    self.bacteria.remove(bact)
        current_bacteria_population=len(self.bacteria)
        pop_density=current_bacteria_population/self.max_pop
        for bact in self.bacteria:
                if current_bacteria_population<=self.max_pop:
                    try:
                        new_bacteria=bact.reproduce(pop_density)
                        self.bacteria.append(new_bacteria)
                        current_bacteria_population=len(self.bacteria)
...
def simulation_with_antibiotic(num_bacteria,
    ...
    resistant_pop_1=[]
    for trial in range(num_trials):
        bacteria_count_ntimestep_1=[]
        rbacteria_count_timestep_1=[]
        ...
        for n in range(150):
            if n==0:
                bacteria_count_ntimestep_1.append(num_bacteria_2)
                rbacteria_count_timestep_1.append(0)
            else:
                num_bacteria_2=patient.update()
                rbacteria_count_timestep_1.append(patient.get_resist_pop())
                bacteria_count_ntimestep_1.append(num_bacteria_2)
        ...
        for n in range(250):
                ...
                rbacteria_count_timestep_1.append(patient.get_resist_pop())
                bacteria_count_ntimestep_1.append(num_bacteria_2)
        populations_1.append(bacteria_count_ntimestep_1)
        resistant_pop_1.append(rbacteria_count_timestep_1)
    avg_pop_1=[calc_pop_avg(populations_1,n) for n in range(400)]
    avg_rpop_1=[calc_pop_avg(resistant_pop_1,n) for n in range(400)]
    make_two_curve_plot(np.array(range(400)),
                        np.array(avg_pop_1),
                        np.array(avg_rpop_1),
                        "Total",
                        "Resistant",
                        ...
                        "With an Anitbiotic")
    print(populations_1)
    print(resistant_pop_1)
    return ((populations_1,resistant_pop_1))
...
total_pop, resistant_pop = simulation_with_antibiotic(num_bacteria=100,
                                                      ...
                                                      mut_prob=0.8,
                                                      num_trials=50)
std_pop_1=calc_95_ci(total_pop, 299)
std_rpop_1=calc_95_ci(resistant_pop, 299)
total_pop, resistant_pop = simulation_with_antibiotic(num_bacteria=100,
                                                      max_pop=1000,
                                                      birth_prob=0.17,
                                                      death_prob=0.2,
                                                      ...
                                                      mut_prob=0.8,
                                                      num_trials=50)
std_pop_2=calc_95_ci(total_pop, 299)
std_rpop_2=calc_95_ci(resistant_pop, 299)
print("The mean,confidence interval for first simulation/second simulations and total population and resistant population respectively ")
print   ( std_pop_1 + std_rpop_1 + std_pop_2 + std_rpop_2)
###########the final findings:
##########(529.5, 6.729453850053509, 529.5, 6.729453850053509, 33.8, 6.713851186912024, 33.8, 6.713851186912024)-means and confidence interval for different simulations
#############The chances are great for the bacteria to become resistant to our drug
